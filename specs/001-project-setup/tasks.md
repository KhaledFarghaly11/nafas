...
- [x] T016 [P] Create navigation types — create `src/types/navigation.ts` with: `CustomerTab` type = `'home' | 'favorites' | 'orders' | 'profile'`, `ChefTab` type = `'dashboard' | 'orders' | 'menu' | 'schedule' | 'stats'`. Export both types. See `specs/001-project-setup/contracts/navigation.md` for tab definitions.
