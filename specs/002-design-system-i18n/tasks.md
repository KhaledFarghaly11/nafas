...
**Input**: Design documents from `/specs/002-design-system-i18n/`
...
- [x] T001 Install new dependencies: run `npm install i18next react-i18next expo-font expo-updates @expo-google-fonts/tajawal @expo-google-fonts/noto-sans-arabic @expo-google-fonts/cormorant-garamond` in project root. Use `--legacy-peer-deps` if needed due to React 19 peer dependency conflicts. Verify no peer dependency warnings in output.
- [x] T002 [P] Font files are now handled by `@expo-google-fonts` packages — no manual TTF downloads needed. Remove any old font files from `assets/fonts/` (SpaceGrotesk-_.ttf, Inter-_.ttf, Cairo-\*.ttf) if they exist from a previous setup.
...
- [x] T004 Expand `src/design/tokens.ts` — replace the current minimal token objects with the full Nafas token set. The file must export `lightTokens` and `darkTokens` (both typed as `TokenSet`). Each token set must contain: (1) `colors` object with ALL of these keys: `background`, `text`, `textSecondary`, `primary`, `primaryLight`, `error`, `success`, `warning`, `surface`, `border`, `tabBackground`, `tabBarInactive`, `skeletonBase`, `skeletonHighlight`, `clay`, `saffron`, `cream`, `linen`, `oud`, `bark`, `smoke`, `mint`, `rose`, `warmWhite`, `overlay`; (2) `spacing` object with keys: `xs=4, sm=8, md=12, lg=16, xl=24, 2xl=32, 3xl=48`; (3) `typography` object with keys `xs, sm, md, lg, xl, 2xl, 3xl, 4xl, numeral`, each containing `{ fontSize, lineHeight, fontFamily }` — fontFamily should be `'NotoSansArabic'` for body sizes, `'TajawalMedium'`/`'TajawalBold'`/`'TajawalExtraBold'` for display sizes, `'CormorantGaramond'`/`'CormorantGaramondSemiBold'` for numeral; (4) `radius` object: `none=0, sm=8, md=12, lg=20, xl=24, full=50`; (5) `shadows` object: `none, sm, md, lg` — each with `shadowColor` using warm-tinted `rgba(44, 26, 14, ...)` instead of `#000000`. Light colors: `background=#FAF3E8` (cream), `text=#2C1A0E` (oud), `textSecondary=#6B3F28` (bark), `primary=#C1714F` (clay), `primaryLight=#E8A838` (saffron), `error=#B84C3C`, `success=#5B8C6A`, `warning=#E8A838`, `surface=#F0E6D3` (linen), `border=#D9CCBC`. Dark colors: `background=#1A110A`, `text=#FAF3E8`, `textSecondary=#9E8A78` (smoke), `primary=#C1714F` (clay), `primaryLight=#D4927A` (rose), `surface=#261A10`, `border=#3D2C1E`. Also export `type TokenSet` for use by theme provider and components.
...
- [x] T008 [P] Create `src/i18n/en/common.json` — English translation file with initial keys: `{ "app_name": "Nafas", "language_arabic": "العربية", "language_english": "English", "language_change_confirm_title": "Change Language", "language_change_confirm_message": "The app will reload to apply the new language. Continue?", "language_change_confirm_ok": "Change", "language_change_confirm_cancel": "Cancel", "theme_light": "Light", "theme_dark": "Dark", "theme_system": "Default", "error_title": "Something went wrong", "error_message": "Try again — something happened", "error_retry": "Try again", "empty_title": "Nothing here yet", "toast_dismissed": "Dismissed", "loading": "Just a moment...", "order_placed": "Done! The cook knows", "empty_cart": "Your cart is empty", "no_cooks": "No cooks available right now — try tomorrow morning", "available_now": "Available now", "loved": "Loved", "home": "Home", "favorites": "Favorites", "orders": "Orders", "profile": "Profile", "cart": "Cart", "search_placeholder": "Search homemade food...", "sign_out": "Sign Out", "dashboard": "Dashboard", "menu": "Menu", "schedule": "Schedule", "stats": "Stats", "in_kitchen": "In the kitchen", "ready": "Ready & covered", "with_driver": "With the driver", "delivered": "Delivered!", "confirming": "Confirming", "currency": "EGP" }`.
- [x] T009 [P] Create `src/i18n/ar/common.json` — Arabic translation file using Egyptian dialect (عامية مصرية), NOT Modern Standard Arabic. Same keys as English: `{ "app_name": "نَفَس", "language_arabic": "العربية", "language_english": "English", "language_change_confirm_title": "تغيير اللغة", "language_change_confirm_message": "التطبيق هيعيد تشغيل عشان يطبق اللغة الجديدة. تكمل؟", "language_change_confirm_ok": "غيّر", "language_change_confirm_cancel": "كانسل", "theme_light": "فاتح", "theme_dark": "داكن", "theme_system": "الافتراضي", "error_title": "في مشكلة صغيرة", "error_message": "حاول تاني — فيه حاجة حصلت", "error_retry": "حاول تاني", "empty_title": "فاضي لسه", "toast_dismissed": "تمام", "loading": "ثانية وهنوريك", "order_placed": "تمام! الطباخة عرفت", "empty_cart": "السلة فاضية لسه", "no_cooks": "معفيش طباخين دلوقتي — جرب بكرة الصبح", "available_now": "متاحة الآن", "loved": "محبوب", "home": "البيت", "favorites": "المفضلة", "orders": "الطلبات", "profile": "الحساب", "cart": "السلة", "search_placeholder": "دوّر على أكل بيتي...", "sign_out": "خرج", "dashboard": "لوحة التحكم", "menu": "المنيو", "schedule": "مواعيد", "stats": "إحصائيات", "in_kitchen": "في المطبخ", "ready": "جاهز ومتغطي", "with_driver": "مع المندوب", "delivered": "وصل!", "confirming": "جاري التأكيد", "currency": "ج.م" }`.
...
- [x] T011 Create `src/design/typography.ts` — export font family definitions used by typography tokens. Define: `export const fontFamilies = { display: 'Tajawal', displayMedium: 'TajawalMedium', displayBold: 'TajawalBold', displayExtraBold: 'TajawalExtraBold', body: 'NotoSansArabic', bodyLight: 'NotoSansArabicLight', bodyMedium: 'NotoSansArabicMedium', numeral: 'CormorantGaramond', numeralSemiBold: 'CormorantGaramondSemiBold', numeralItalic: 'CormorantGaramondItalic' } as const`. Export a type `FontRole = 'display' | 'body' | 'numeral'` and helper `getFontFamily(role: FontRole): string` that returns the base font family for the given role. Font family resolution is now embedded in typography tokens directly — this file serves as a reference and type definition.
- [x] T012 Update `app/_layout.tsx` — integrate Google Fonts loading, i18n initialization, and settings store sync. (1) Add imports from `@expo-google-fonts/tajawal` (Tajawal_400Regular, Tajawal_500Medium, Tajawal_700Bold, Tajawal_800ExtraBold), `@expo-google-fonts/noto-sans-arabic` (NotoSansArabic_300Light, NotoSansArabic_400Regular, NotoSansArabic_500Medium), `@expo-google-fonts/cormorant-garamond` (CormorantGaramond_400Regular, CormorantGaramond_600SemiBold, CormorantGaramond_400Regular_Italic). (2) Add import of `useFonts` from `expo-font`. (3) Add import of `i18n` from `@/i18n`. (4) Add import of `useSettingsStore` from `@/stores/settings-store`. (5) In `RootLayout`, call `const [fontsLoaded] = useFonts({ Tajawal: Tajawal_400Regular, TajawalMedium: Tajawal_500Medium, TajawalBold: Tajawal_700Bold, TajawalExtraBold: Tajawal_800ExtraBold, NotoSansArabic: NotoSansArabic_400Regular, NotoSansArabicLight: NotoSansArabic_300Light, NotoSansArabicMedium: NotoSansArabic_500Medium, CormorantGaramond: CormorantGaramond_400Regular, CormorantGaramondSemiBold: CormorantGaramond_600SemiBold, CormorantGaramondItalic: CormorantGaramond_400Regular_Italic })` — font map keys MUST match the fontFamily values in `tokens.ts` typography entries. (6) Add `const settingsHydrated = useSettingsStore(s => s.hydrated)`. (7) Change the `if (!hydrated)` check to `if (!hydrated || !settingsHydrated || !fontsLoaded)` — return null until all three are ready. (8) After the hydration check, add a `useEffect` that syncs i18n language with settings store: `useEffect(() => { const lang = useSettingsStore.getState().language; if (i18n.language !== lang) { i18n.changeLanguage(lang); } }, [settingsHydrated])`. (9) Wrap children with `<ToastProvider>` inside `<ThemeProvider>`: `<QueryProvider><ThemeProvider><ToastProvider><Slot /></ToastProvider></ThemeProvider></QueryProvider>`.
...
- [x] T013 [US1] Create `src/components/primitives/Text.tsx` — themed Text component. Props per contracts: `children: ReactNode`, `variant?: 'heading1'|'heading2'|'heading3'|'body'|'caption'|'overline'|'price'` (default `'body'`), `color?: 'primary'|'secondary'|'error'|'inverse'|'clay'|'saffron'|'mint'|'smoke'` (default `'primary'`), `align?: 'left'|'center'|'right'|'auto'` (default `'auto'`), `numberOfLines?: number`, `style?: TextStyle`, `testID?: string`. Implementation: (1) Call `useTheme()` to get tokens. (2) Map variant to typography token: heading1→4xl, heading2→3xl, heading3→2xl, body→md, caption→xs, overline→xs, price→numeral. (3) Map color prop to token: primary→colors.text, secondary→colors.textSecondary, error→colors.error, inverse→colors.background, clay→colors.clay, saffron→colors.saffron, mint→colors.mint, smoke→colors.smoke. (4) For `align='auto'`, use `useRTL()` to set `textAlign` to `'right'` if RTL else `'left'`. (5) Font family comes directly from the typography token's fontFamily field — no dynamic language-based resolution needed since tokens already embed the correct font per variant. Export as named export.
- [x] T014 [US1] Create `src/components/primitives/Button.tsx` — themed Button component. Props per contracts: `title: string`, `onPress: () => void`, `variant?: 'primary'|'secondary'|'ghost'|'danger'` (default `'primary'`), `size?: 'sm'|'md'|'lg'` (default `'md'`), `disabled?: boolean`, `loading?: boolean`, `leftIcon?: string`, `rightIcon?: string`, `style?: ViewStyle`, `testID?: string`. Implementation: (1) Call `useTheme()` for tokens, `useRTL()` for direction. (2) Map variant to colors: primary→bg=colors.clay text=colors.warmWhite (via 'inverse'), secondary→bg=colors.surface text=colors.text border=colors.border, ghost→bg=transparent text=colors.text, danger→bg=colors.error text=colors.warmWhite (via 'inverse'). (3) Map size to padding: sm→spacing.sm, md→spacing.md, lg→spacing.lg. (4) Map size to text variant: sm→caption, md→body, lg→heading3. (5) When `loading=true`, show `<ActivityIndicator>` in matching color instead of title text, and set `pointerEvents='none'`. (6) When `disabled=true`, reduce opacity to 0.5 and set `pointerEvents='none'`. (7) Render left/right Icon components if provided. (8) Wrap everything in a `<Pressable>` — use function-style `style` callback to apply `pressed` state with `transform: [{ scale: pressed ? 0.97 : 1 }]` for subtle press feedback. (9) Border radius uses `tokens.radius.sm` (8px). Export as named export.
- [x] T015 [US1] Create `src/components/primitives/Icon.tsx` — themed Icon component. Props: `name: string` (Feather icon name), `size?: number` (default 24), `color?: 'primary'|'secondary'|'error'|'inverse'|'clay'|'saffron'|'mint'|'smoke'|'bark'` (default `'bark'`), `mirrorInRTL?: boolean` (default false), `style?: ViewStyle`, `testID?: string`. Implementation: (1) Import `Feather` from `@expo/vector-icons`. (2) Call `useTheme()` to map color prop to token value — primary→colors.text, secondary→colors.textSecondary, error→colors.error, inverse→colors.background, clay→colors.clay, saffron→colors.saffron, mint→colors.mint, smoke→colors.smoke, bark→colors.bark. (3) Call `useRTL()` and `applyRTLStyle()` from `src/lib/rtl.ts` — if `mirrorInRTL && isRTL`, apply `scaleX: -1` transform. (4) Render `<Feather name={name} size={size} color={resolvedColor} style={[rtlStyle, style]} />`. Default color is `bark` (Nafas medium brown for icons) instead of generic text color. Export as named export.
...
- [x] T028 [US3] Create barrel exports file `src/components/primitives/index.ts` — re-export all primitives: `export { Text } from './Text'`, `export { Button } from './Button'`, `export { Icon } from './Icon'`, `export { Card } from './Card'`, `export { Input } from './Input'`, `export { Badge } from './Badge'`, `export { Divider } from './Divider'`.
- [x] T029 [P] [US3] Create barrel exports file `src/components/feedback/index.ts` — re-export all feedback components: `export { Skeleton } from './Skeleton'`, `export { EmptyState } from './EmptyState'`, `export { ErrorState } from './ErrorState'`, `export { ToastProvider, useToast } from './Toast'`.
- [x] T030 [P] [US3] Create barrel exports file `src/components/layout/index.ts` — re-export: `export { ScreenContainer } from './ScreenContainer'`.
- [x] T031 [US3] Create barrel exports file `src/components/index.ts` — re-export all component groups: `export * from './primitives'`, `export * from './feedback'`, `export * from './layout'`.
